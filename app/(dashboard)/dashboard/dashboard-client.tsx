...
import { TrendingUp, TrendingDown, Wallet, Sparkles, ArrowRight, Calendar, Target } from 'lucide-react'
...
interface DashboardClientProps {
  ...
  initialExpenses: Array<{ _id: string; amount: number; currency: string; category: string }>
  initialTotalExpenses: Array<{ _id: string; total: number }>
  initialExpensesByCategory: Array<{ _id: { category: string; currency: string }; total: number }>
  initialMonth: number
  initialYear: number
}
...
export function DashboardClient({
  ...
  const usagePercent = totalIncome > 0 ? ((totalExpenses / totalIncome) * 100).toFixed(1) : '0'
    ...
    <div className="space-y-6">
      ...
      <Card className="border-primary/20 bg-card">
        <CardContent className="flex items-center gap-4 py-4">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-wider text-primary">Financial Fun Fact</p>
            <p className="text-sm font-semibold text-foreground">{randomFact}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="border-border bg-card">
          <CardContent className="pt-6">
            ...
            <p className="mt-2 text-2xl font-bold text-income">{formatAmount(totalIncome)}</p>
            <p className="text-xs text-muted-foreground">{months[initialMonth - 1]} {initialYear}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card">
          <CardContent className="pt-6">
            ...
            <p className="mt-2 text-2xl font-bold text-expense">{formatAmount(totalExpenses)}</p>
            <p className="text-xs text-muted-foreground">{months[initialMonth - 1]} {initialYear}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card">
          <CardContent className="pt-6">
            ...
            <p className="mt-2 text-2xl font-bold text-foreground">{formatAmount(balance)}</p>
            <div className="mt-2 flex items-center gap-2">
              <p className="text-xs text-muted-foreground">Usage Flow</p>
              <span className="text-xs font-semibold text-primary">{usagePercent}%</span>
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-bold uppercase tracking-wider">Expenditure Flow</CardTitle>
              <p className="text-xs text-muted-foreground">Monthly Category Analysis</p>
            </div>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-bold uppercase tracking-wider">Market Share</CardTitle>
              <p className="text-xs text-muted-foreground">Total Spent Allocation</p>
            </div>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-bold uppercase tracking-wider">Upcoming Records</CardTitle>
              <p className="text-xs text-muted-foreground">Bills & Dues Due Soon</p>
            </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between rounded-lg bg-secondary/50 p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold">House Rent <Badge variant="secondary" className="ml-2 text-[10px]">BILL</Badge></p>
                  <p className="text-xs text-muted-foreground">Jun 01, {initialYear}</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between rounded-lg bg-secondary/50 p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold">Electricity <Badge variant="secondary" className="ml-2 text-[10px]">BILL</Badge></p>
                  <p className="text-xs text-muted-foreground">Jun 15, {initialYear}</p>
                </div>
              </div>
              <div className="text-right">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-bold uppercase tracking-wider">Savings Goals</CardTitle>
            </div>
            <Badge className="bg-income/20 text-income text-[10px]">ACTIVE</Badge>
          </CardHeader>
          <CardContent className="flex flex-col items-center justify-center py-8">
            <Target className="h-16 w-16 text-muted-foreground/30" />
            <p className="mt-4 text-sm text-muted-foreground">No savings goals set</p>
            <p className="text-xs text-muted-foreground">Start tracking your savings</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-primary/20 bg-card">
        <CardHeader className="pb-2">
          <div className="flex items-center gap-3">
            <Sparkles className="h-5 w-5 text-primary" />
            <div>
              <CardTitle className="text-sm font-bold uppercase tracking-wider">AI Financial Assistant</CardTitle>
              <p className="text-xs text-muted-foreground">Smart health check for {months[initialMonth - 1]} {initialYear}</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <Card className="mt-4 border-border bg-secondary/30">
            <CardContent className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Ready for Deeper Analysis?</p>
                  <p className="text-sm font-semibold">Unlock Exhaustive Financial Intelligence</p>
                </div>
              </div>
              <Button variant="ghost" className="text-sm">
                Analyze Deep Strategic Depth <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </CardContent>
          </Card>
        </CardContent>
